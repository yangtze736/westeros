...
bool MiddleWare::parseFromStr(const std::string &str, const char *key, std::string &value)
{
	...
	std::string::size_type pos1 = str.find(target1);
	if(pos1 != std::string::npos)
	{
		size_t offset = target1.length();

		std::string::size_type pos2 = str.find(split,pos1+offset);
	...
	{
		std::string::size_type pos3 = str.find(target2);
		if(pos3 != std::string::npos)
		{
			size_t offset = target2.length();

			std::string::size_type pos4 = str.find(split,pos3+offset);
