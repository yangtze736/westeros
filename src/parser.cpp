...
// Static Interface
bool Parser::genMetaData(std::string &genJson, const std::string &fileSize, const std::string &time)
{
	Json::Value root;

	root["size"] = fileSize;
	root["time"] = time;
	genJson = root.toStyledString();
...
bool Parser::parseMergeFile(std::string &token, std::string &uuid, std::string &dest, std::string &flag, std::string &postField, const std::string &strJson)
{
	...
	reader.parse(strJson, value);
	flag = value["X-Static-Large-Object"].asString();
	token = value["X-Auth-Token"].asString();
	uuid = value["uuid"].asString();
